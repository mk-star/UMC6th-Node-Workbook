...
import { getReview, getMission } from "../providers/user.provieder.js";
...
export const reviewPreview = async (req, res, next) => {
  console.log("사용자가 작성한 리뷰 목록을 요청하였습니다!");
...
export const missionPreview = async (req, res, next) => {
  console.log("사용자가 진행 중인 미션 목록을 요청하였습니다!");
  console.log("query:", req.query);

  res.send(
    response(status.SUCCESS, await getMission(req.params.memberId, req.query))
  );
};
